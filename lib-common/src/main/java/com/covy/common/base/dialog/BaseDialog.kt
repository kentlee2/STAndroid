package com.covy.common.base.dialog
...
import com.covy.common.R
import com.covy.common.callback.action.AnimAction
import com.covy.common.callback.action.HandlerAction
...
open class BaseDialog constructor(context: Context, @StyleRes themeResId: Int = R.style.BaseDialogTheme) :
    AppCompatDialog(context, themeResId), LifecycleOwner,
    AnimAction, HandlerAction, DialogInterface.OnShowListener,
    ...
    override fun dismiss() {
        ...
        if (focusView != null) {
            getSystemService(context,InputMethodManager::class.java)?.hideSoftInputFromWindow(focusView.windowToken, 0)
        }
        super.dismiss()
    }
    ...
    open class Builder<B : Builder<B>> constructor(val context: Context) {
        ...
        open fun setGravity(gravity: Int): B {
            // 适配布局反方向
            this.gravity = Gravity.getAbsoluteGravity(gravity, context.resources.configuration.layoutDirection)
            if (isCreated()) {
        ...
        open fun setText(@IdRes viewId: Int, @StringRes stringId: Int): B {
            return setText(viewId, context.getString(stringId))
        }
        ...
        open fun setHint(@IdRes viewId: Int, @StringRes stringId: Int): B {
            return setHint(viewId, context.getString(stringId))
        }
